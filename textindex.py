...
class VisionApi:
    ...
    def detect_text(self, input_filenames, num_retries=3, max_results=200):
...
class Index:
    ...
    def __init__(self,
                 ...
                 stopwords=nltk.corpus.stopwords.words('indonesia')):
    ...
    def add(self, filename, document):
        ...
        # image.
        #print("Documents found in %s: %s" % (filename, document))
        #counter = 1
        #for token in [t.lower() for t in nltk.word_tokenize(document)]:
        for token in [t.lower() for t in re.split(':|;|,| |\n', document)]:
